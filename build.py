...
def err(m):   print(f"{C.RED}[ERR ]{C.RESET}  {m}")
...
def run(cmd, cwd=None, check=True, capture=False, env=None):
    ...
    return subprocess.run(cmd, cwd=cwd, check=check,
                          capture_output=capture, text=True, env=merged)
...
def prebuild(platform: str) -> dict:
    ...
    return {"version_name": display, "version_code": version_code, "version_full": version_full}
...
def sed_rename(args):
    ...
    native_globs = [
        ...
        "android/**/*.xml",  "android/**/*.gradle",
        "android/**/*.gradle.kts", "android/**/*.properties",
        "ios/**/*.plist",    "ios/**/*.pbxproj",
        "macos/**/*.plist",  "macos/**/*.pbxproj",
        "linux/**/*.cmake",  "linux/**/*.desktop",
        "windows/runner/*.rc", "windows/runner/*.h",
        "windows/runner/*.cpp",
    ]
...
def apply_patches(root: str, platform: str = ""):
    ...
    for name in ["bottom_sheet_patch.diff", "modal_barrier_patch.diff", "mouse_cursor_patch.diff"]:
        p = Path("lib/scripts") / name
        if not p.exists():
            warn(f"patch 不存在，跳过: {name}"); continue
        try:
...
def setup_signing(args) -> bool:
    ...
    if not args.key_alias:
        err("签名需要 --key-alias"); sys.exit(1)
    if not args.key_password:
        err("签名需要 --key-password"); sys.exit(1)
...
def build_android(args, vi: dict):
    ...
    try:
        run([
            ...
            *dart_defs(args), *(args.extra_build_args or []),
        ])
    finally:
...
def build_ios(args, vi: dict):
    if _sys_platform.system() != "Darwin":
        err("iOS 构建只能在 macOS"); sys.exit(1)
    step("构建 iOS")
    ...
    sh('find Payload/Runner.app/Frameworks -type d -name "*.framework" '
...
def build_macos(args, vi: dict):
    if _sys_platform.system() != "Darwin":
        err("macOS 构建只能在 macOS"); sys.exit(1)
    step("构建 macOS")
...
def build_windows(args, vi: dict):
    ...
    if args.installer and shutil.which("fastforge"):
        dfs = (f"dart-define-from-file={args.dart_define_from_file}"
               if args.dart_define_from_file and Path(args.dart_define_from_file).exists() else "")
...
def _arch_pkg(prefix, version, arch, bundle, o, args):
    ...
    desktop = next(Path("assets/linux").glob("*.desktop"), None) \
              if Path("assets/linux").exists() else None
    ...
    # ── 写 PKGBUILD ──
    (work / "PKGBUILD").write_text(textwrap.dedent(f"""\
    ...
    # ── 运行 makepkg ──
    sh("makepkg -f --noconfirm", cwd=work, env={
        **os.environ,
...
def _deb(prefix, version, arch, bundle, o, args):
    ...
    for d in ["opt/app", "usr/share/applications",
    ...
    if ctrl_src.exists():
        ...
        if ctrl.exists():
            ...
            size_kb = sum(
                f.stat().st_size for f in (root / "opt/app").rglob("*") if f.is_file()
            ) // 1024 + 1
    ...
    logo = Path("assets/images/logo/logo.png")
    if logo.exists():
        shutil.copy2(logo, root / f"usr/share/icons/hicolor/512x512/apps/{app_name}.png")
...
def _rpm(prefix, version, arch, bundle, o, args):
    ...
    sh(f"tar -zcf {rpm_root}/SOURCES/{app_name}-{rpm_ver}.tar.gz "
       f"-C {rpm_root} {app_name}-{rpm_ver}")

    desktop_name = desktop.name if desktop else f"com.example.{app_name}.desktop"
    date_str = datetime.datetime.now().strftime("%a %b %d %Y")

    (rpm_root / f"SPECS/{app_name}.spec").write_text(textwrap.dedent(f"""\
        Name:           {app_name}
        Version:        {rpm_ver}
        Release:        1%{{?dist}}
        Summary:        Flutter App
...
def _appimage(prefix, version, arch, bundle, o, args):
    ...
    if not tool.exists():
        sh("wget -q https://github.com/AppImage/appimagetool/releases/"
    ...
    for d in ["usr/bin", "usr/lib", "usr/share/applications",
              "usr/share/icons/hicolor/512x512/apps"]:
    ...
    if logo.exists():
        ...
        shutil.copy2(logo, appdir / f"usr/share/icons/hicolor/512x512/apps/{app_name}.png")
...
def build_linux(args, vi: dict):
    if _sys_platform.system() != "Linux":
        err("Linux 构建只能在 Linux"); sys.exit(1)
    step("构建 Linux")
    ...
    if not bundle.exists():
        err(f"未找到构建产物: {bundle}"); sys.exit(1)
...
def parse_args():
    ...
    g.add_argument("--pkg-id",            metavar="ID",
                   help="新 Bundle ID，如 com.myfork.app")
    g.add_argument("--original-pkg-id",   metavar="ID",  default="com.example.pilisuper",
                   help="上游 applicationId（默认: com.example.pilisuper）")
    g.add_argument("--app-name",          metavar="NAME",
                   help="新 pubspec name / 显示名")
    g.add_argument("--original-app-name", metavar="NAME", default="pilisuper",
                   help="上游 pubspec name（默认: pilisuper）")
    g.add_argument("--skip-rename",       action="store_true")

    g = p.add_argument_group("构建参数")
    g.add_argument("--dart-define-from-file", default="pili_release.json", metavar="FILE")
    ...
    g.add_argument("--clean-keys",      action="store_true",
    ...
    g.add_argument("--linux-targets", nargs="+", default=["tar.gz"],
...
def main():
    if not Path("pubspec.yaml").exists():
        err("请在 Flutter 项目根目录运行（找不到 pubspec.yaml）"); sys.exit(1)
